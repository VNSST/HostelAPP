...
export default function CashApprovals() {
  ...
  return (
    <div>
        ...
        {cashPending.map(p => (
          <div className="card" key={p.id}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button className="button success" style={{ flex: 1, fontSize: '0.85rem' }} disabled={actionLoading === p.id} onClick={() => handleAction(p.id, 'confirm-cash')}>
      ...
      {/* UPI Pending Section */}
      <h2 style={{ marginBottom: '1rem', fontSize: '1.1rem' }}>📱 UPI Payments Awaiting Reconciliation ({upiPending.length})</h2>
          ...
          <tbody>
            {upiPending.map(p => (
              <tr key={p.id}>
                ...
                <td><code style={{ background: '#F3F4F6', padding: '0.15rem 0.4rem', borderRadius: '4px', fontSize: '0.8rem' }}>{p.upi_reference_id || '—'}</code></td>
                <td>{p.screenshot_url ? <a href={`http://localhost:5000${p.screenshot_url}`} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary)' }}>View</a> : '—'}</td>
                <td>{new Date(p.payment_date).toLocaleDateString('en-IN')}</td>
              </tr>
            ))}
            {upiPending.length === 0 && (
              <tr><td colSpan="6" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem' }}>No pending UPI payments. Upload a bank CSV under "Bank Reconcile" to verify.</td></tr>
            )}
          </tbody>
        </table>
      </div>
