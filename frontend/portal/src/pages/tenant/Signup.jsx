...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    if (!formData.room_number) return setError('Please select a room');
    ...
    try {
      ...
      navigate('/tenant/login', { state: { registered: true } });
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
    ...
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
      <div className="card" style={{ width: '100%', margin: 0 }}>
        <div style={{textAlign: 'center', marginBottom: '1.5rem'}}>
          ...
          <h1 style={{ color: 'var(--primary)', margin: 0, fontSize: '1.5rem' }}>Join Hostel</h1>
        ...
        {step === 2 && (
          <form onSubmit={handleSignup}>
            ...
            <div className="form-group">
              ...
              <div style={{display: 'flex', gap: '0.5rem'}}>
                ...
                <button type="button" className="button outline" onClick={async () => {
                  if (!formData.hostel_unique_username) return;
                  setFetchingRooms(true);
                  try {
                    const res = await fetch(`${API_BASE}/rooms/public/${formData.hostel_unique_username}`);
                    const data = await res.json();
                    if (!res.ok) throw new Error(data.error);
                    setRooms(data);
                    if (data.length === 0) setError('No rooms found for this hostel');
                    else setError('');
                  } catch (err) { setError(err.message); }
                  setFetchingRooms(false);
                }} disabled={fetchingRooms}>{fetchingRooms ? '...' : 'Find Rooms'}</button>
              </div>
              <span style={{display: 'block', fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.3rem'}}>Enter username and click Find Rooms to load available rooms</span>
            </div>
            ...
            <div className="form-group">
              ...
              <select id="tenant-room" value={formData.room_number} onChange={e => handleChange('room_number', e.target.value)} required>
                <option value="">Select a room</option>
                {rooms.map(r => (
                  <option key={r.room_number} value={r.room_number}>{r.room_number} (₹{r.rent_amount})</option>
                ))}
              </select>
            </div>
        ...
        <p className="auth-toggle">
          Already have an account? <Link to="/tenant/login">Sign In</Link>
        </p>
      </div>
    </div>
  );
