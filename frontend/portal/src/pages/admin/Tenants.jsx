...
export default function Tenants() {
  ...
  const handleRemoveTenant = async (id) => {
    if (!window.confirm('Are you sure you want to remove this tenant? Their payment history will also be deleted.')) return;
    try {
          ...
          <tbody>
            {tenants.map(t => (
              <tr key={t.id}>
                ...
                <td>
                  <button className="button outline danger" style={{padding: '0.3rem 0.6rem', fontSize: '0.75rem'}} onClick={() => handleRemoveTenant(t.id)}>
                    Remove
                  </button>
                </td>
              </tr>
            ...
            {tenants.length === 0 && (
              <tr><td colSpan="8" style={{textAlign: 'center', color: 'var(--text-muted)', padding: '2rem'}}>No tenants have registered yet. Share your hostel username with tenants so they can sign up.</td></tr>
            )}
          </tbody>
        </table>
      </div>
